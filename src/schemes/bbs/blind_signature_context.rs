...
impl BlindSignatureContext {
    ...
    /// Needs (N + 1) * 32 + 48 space otherwise it will panic
    pub fn to_bytes(&self) -> Vec<u8> {
        let size = COMMITMENT_G1_BYTES + FIELD_BYTES * (self.proofs.len() + 1);
        let mut buffer = Vec::with_capacity(size);
        buffer.extend_from_slice(&self.commitment.to_bytes());
    ...
    pub fn verify(
        ...
        let mut known = HashSet::with_capacity(known_messages.len());
        let mut points = Vec::with_capacity(2 + generators.len() - known_messages.len());
        ...
        points.extend_from_slice(&[generators.h0, self.commitment.0]);
